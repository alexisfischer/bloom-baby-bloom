...
clear;
filepath='~/MATLAB/bloom-baby-bloom/IFCB-Data/Shimada/';
outpath = '~/MATLAB/NOAA/Shimada/'; 
addpath(genpath(outpath));
addpath(genpath('~/MATLAB/ifcb-analysis/')); 
addpath(genpath('~/MATLAB/bloom-baby-bloom/')); 

load([filepath 'class/summary_allTB_2019'],'filelistTB','class2useTB',...
    'ml_analyzedTB','mdateTB','classcountTB_above_optthresh'); % ADF note: be sure to use classcountTB_above_optthresh for classifier output
load([filepath 'manual/count_class_manual_17Aug2021.mat'],...
...
[~,class_label]=get_phyto_ind_PNW(class2useTB); 
...
ax=subplot(4,5,11); ax.YLabel.String='Automated Counts'; ax.YLabel.FontSize=16;
ax=subplot(4,5,18); ax.XLabel.String='Manual Counts'; ax.XLabel.FontSize=16;

% set figure parameters
set(gcf,'color','w');
print(gcf,'-dtiff','-r300',[outpath 'Figs/Class_vs_Manual_Scatterplot.tif']);
hold off
