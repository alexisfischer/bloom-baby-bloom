...
if yr==2019
    ...
    P=P((P.LAT>43 & P.LAT<44.7),:); %only look at Heceta Bank
    top_percentage = 0.25; % l+m
    bandwidth_lat = 0.6; % l+m
    bandwidth_data = 0.7; % l+m     
elseif yr==2021
    P((P.DT<datetime('01-Jan-2020')),:)=[];
    P=P((P.LAT>47.5),:); %only look at Heceta Bank    
    top_percentage = 0.25; % l+m
    bandwidth_lat = 0.6; % l+m
...
clearvars idx bandwidth* lat_range_highest_data %range

%%%% Plot the density estimate
fig=figure; set(gcf,'color','w','Units','inches','Position',[1 1 3.7 3.5]); 

contourf(grid_lat_pn, grid_pn, density_pn', 'LineStyle', 'none'); hold on;
plot(lat,data,'k.','linewidth',1,'markersize',4)
plot(lat_range_pn(1,:),[0 0],'r',lat_range_pn(2,:),[0 0],'r','LineWidth', 6);    

title([num2str(yr) ' (n=' num2str(length(data)) ')']);
xlabel('Temperature');
ylabel('log (PN) cells/mL');
colorbar;
set(gca,'xlim',[9.5 17],'xtick',10:2:18,'fontsize',11,'tickdir','out'); hold on;
hold off;

if fprint==1
    exportgraphics(fig,[filepath 'NOAA/Shimada/Figs/KDE_' num2str(yr) 'temp.png'],'Resolution',100)    
end
hold off 
