...
load([filepath 'Data/SoundToxins_Dinophysis.mat'],'QH','BI','LB','SB','S');
%load([filepath 'Data/SequimBay_Dinophysiscells.mat'],'T','SB');

%SB.ComBlo=sum([SB.Bloom,SB.Common],2);

figure('Units','inches','Position',[1 1 3.5 4.2],'PaperPositionMode','auto');
subplot = @(m,n,p) subtightplot (m, n, p, [0.04 0.04], [0.08 0.17], [0.2 0.04]);
%subplot = @(m,n,p) subtightplot(m,n,p,opt{:}); 
%where opt = {gap, width_h, width_w} describes the inner and outer spacings.
c=brewermap(length(S),'Spectral');

subplot(2,1,1)
b=bar(BI.Month,[BI.n,SB.n,QH.n,LB.n],'stacked','FaceColor','flat','BarWidth',1); hold on;
for i=1:length(b)
    set(b(i),'FaceColor',c(i,:))
end
set(gca,'ylim',[0 150],'ytick',0:50:150,'fontsize',10,...
    'xlim',[.5 12.5],'xtick',1:1:12,'tickdir','out','xticklabel',{})
ylabel('total samples','fontsize',11)

lh=legend('Budd Inlet','Sequim Bay',...
    'Quartermaster Harbor','Liberty Bay','Location','North','fontsize',10); legend boxoff;
    lh.FontSize = 9; hp=get(lh,'pos');
    lh.Position=[hp(1) hp(2)+.19 hp(3) hp(4)]; hold on   
...
plot(LB.Month,LB.Present,':',LB.Month,(LB.Common+LB.Bloom),'-','Color',c(4,:),'markersize',3,'linewidth',2); hold on
