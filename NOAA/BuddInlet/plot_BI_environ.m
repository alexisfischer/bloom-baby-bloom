...
load([filepath 'Data/BuddInlet_data_summary'],'D');
...
subplot(4,1,1);
yyaxis left
idx=~isnan(D.dinoML_microscopy); 
plot(D.dt(idx),D.dinoML_microscopy(idx),'ko-','MarkerSize',4);
...
yyaxis right
plot(D.dt,D.DST,'r*','MarkerSize',5);
    ...
    datetick('x', 'mmm', 'keeplimits');   hold on 

ax(3)=subplot(4,1,2); %chl
pcolor(X,Y,C); cax=[0 10]; clim(cax); shading flat; 
