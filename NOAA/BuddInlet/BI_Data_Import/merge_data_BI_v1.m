...
load([filepath 'IFCB-Data/BuddInlet/class/summary_biovol_allTB'],'DcountTB','DesdTB',...
   'DgrayTB','filelistTB','mdateTB','ml_analyzedTB','filecommentTB','runtypeTB');

% adjust data by respective slopes
load([filepath 'IFCB-Data/BuddInlet/class/summary_adjustedTB_Dinophysis'],'slope');
dino=DcountTB./ml_analyzedTB./slope;
...
clearvars classcount ml_analyzed class2use i mesoi im it filelist DesdTB DgrayTB slope

% remove discrete samples (data with file comment)
idx=contains(filecommentTB,'trigger'); 
dino(idx)=[]; dSize(idx)=[]; dGray(idx)=[]; meso(idx)=[];  mdateTB(idx)=[]; filecommentTB(idx)=[]; runtypeTB(idx)=[];

% put in timetable
dt=datetime(mdateTB,'convertfrom','datenum','Format','dd-MMM-yyyy HH:mm:ss');
TT=timetable(dt,dino,dSize,dGray,meso);
