...
%%
% Dinophysis vs Mesodinium cell size
...
yyaxis left    
    ...
    yyaxis right
        hf=plot(fli.dt,fli.mSize,'.','color','r','Linewidth',.5); hold on;
        set(gca,'xgrid','on','ylim',yax,'tickdir','out','xlim',[datetime([char(yrlist(j)) '-05-01']) datetime([char(yrlist(j)) '-10-01'])],...
            'xticklabel',{},'fontsize',10,'ycolor','r'); 
        grid off; 

end
datetick('x', 'mmm', 'keeplimits');            
lh=legend([hd hf hn],'Dinophysis/mL','Mesodinium ESD (\mum)','no IFCB data','location','nw');  hp=get(lh,'pos');
    lh.Position=[hp(1)-.02 hp(2)+.6 hp(3) hp(4)]; hold on    

% set figure parameters
exportgraphics(gcf,[filepath 'NOAA/BuddInlet/Figs/Dinophysis_Mesodinium_size.png'],'Resolution',100)    
hold off
