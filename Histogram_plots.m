...
load ([resultpath 'summary/biovol_size_allcells_05Oct2017']);
...
%% new figure for each sample
%Click enter to see each file go through the figure
figure('Units','inches','Position',[1 1 5 5],'PaperPositionMode','auto');
for i=1:length(matdate)
    bar(diambins,bincount(i,:));
    ...
    t = char(datetime(matdate(i),'ConvertFrom','datenum'));
    title_date=t(1:11);
