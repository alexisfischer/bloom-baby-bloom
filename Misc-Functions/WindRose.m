...
if ~isaxisempty % If axis are specified (not empty)
    ...
    % remove the cricle from the legened information.
...
%% Title and Legend
title(TitleString,'color',TextColor,'fontsize',18,'fontweight',titlefontweight);         % Display a title
...
if LegendType==2                                                           % If legend type is box:
    ...
    l       = legend(axs,leyenda,'location','northwest','fontsize',13);                  % Display the legend wherever (position is corrected)
...
function FrequecyLabels(circles,radius,angulo,TextColor)
    ...
    for i=1:length(circles)
        text(radius(i)*c,radius(i)*s,[num2str(circles(i)) '%'],'HorizontalAlignment',ha,'verticalalignment',va,'color',TextColor,'fontsize',12); % display the labels for each circle
    end
...
function CardinalLabels(circlemax,TextColor,labels)
    text( circlemax,0,[' ' labels.E],'HorizontalAlignment','left'  ,'verticalalignment','middle','color',TextColor,'fontsize',16); % East  label
    text(-circlemax,0,[labels.W ' '],'HorizontalAlignment','right' ,'verticalalignment','middle','color',TextColor,'fontsize',16); % West  label
    text(0, circlemax,labels.N      ,'HorizontalAlignment','center','verticalalignment','bottom','color',TextColor,'fontsize',16); % North label
    text(0,-circlemax,labels.S      ,'HorizontalAlignment','center','verticalalignment','top'   ,'color',TextColor,'fontsize',16); % South label
