function [pars covars SE gval exitflag]=loglogistfitc(x,minimizer,init,options)

%Fits the loglogistic distribution to the data x based on maximum likelihood.
%The data can be left and/or right and/or interval censored.

%The log(pdf) and cdf are respectively  
%logpdf=@(x,g) (((g(1) - log(x))./g(2))  -(log(g(2))+log(x)+2.*log(exp((g(1) - log(x))./g(2)) + 1)));
%cdf=@(x,g) 1 ./ (1 + exp(-(log(x) - g(1)) ./ g(2)));

%where g(1) and g(2) are the parameters to be estimated.

%(This is the parameterization of the pdf also used in SAS and R)
%see the following if you want to use the alternative parametrization:
%http://support.sas.com/documentation/cdl/en/statug/63347/HTML/default/viewer.htm#statug_lifereg_sect019.htm

%g(1) and g(2) are the mu and sigma as they appear in the
%above site (and w=log(x)). In case you need the other parameterization you simply need to
%transform as indicated in the above site. (gamma=1/g(2), a=exp(-g(1)/g(2))
%Results in this function are the same as one would expect by using survreg
%of R by typing:
%survreg(Surv(xlow, xupp, type="interval2") ~ 1, dist="loglogistic")
%or in SAS by typing:
%	proc lifereg data=mydata;
%	model (xlow,xupp)= /dist=loglogistic ;
%	run;
%So you might want to take a look at the corresponding documentation of
%these routines for further information of the parameterization used.
...
xlow=x(:,1);xupp=x(:,2);
if min(xlow)<0;error('The data must be non-negative for the Log-Logistic distribution');end
if min(xupp)<0;error('The data must be non-negative for the Log-Logistic distribution');end
...
if length(find(xlow==-Inf | xupp==Inf))==length(x(:,1));error('Analysis cannot be performed when all data are either left or right censored');end
...
%data. 
loglpdf=@(x,g) (((g(1) - log(x))./g(2))  -(log(g(2))+log(x)+2.*log(exp((g(1) - log(x))./g(2)) + 1)));
lcdf=@(x,g) 1 ./ (1 + exp(-(log(x) - g(1)) ./ g(2)));


lastwarn('')
if sum(xlow==xupp)==length(xlow)  %if all data are exactly observed
    pars=mle(xlow,'distribution','loglogistic');
    logL=@(g) -sum(loglpdf(x,[g(1) g(2)]));  
...
elseif  max(xupp)==Inf && min(xlow)>0 && isequal(xupp(xupp<Inf),xlow(xupp<Inf))==1 %and if there is ONLY right censoring 
       statusupp=(xupp==Inf);
       pars=mle(xupp,'distribution','loglogistic');
       logL=@(g) -sum((statusupp==0).*loglpdf(xupp,[g(1) g(2)])+...
                (statusupp==1).*log(1-lcdf(xupp,[g(1) g(2)])));
       %if isempty(lastwarn)==1;exitflag=1;end
       gval=-logL(pars);%end
       ...
       if isempty(init)==1
            ...
            init=mle(datanew,'distribution','loglogistic');
                        
            
       end
              %this is the general likeelihood:
              logL=@(g) -  ((sum(-1.*(xlow==xupp)+log(lcdf(xupp,[g(1),g(2)]) - lcdf(xlow,[g(1),g(2)])).^(xlow<xupp)))+...
                     sum(-1.*(xlow<xupp)+(loglpdf(xupp,[g(1),g(2)])).^(xlow==xupp)));
