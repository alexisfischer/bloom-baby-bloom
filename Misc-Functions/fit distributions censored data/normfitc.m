function [pars covars SE gval exitflag]=normfitc(x,minimizer,init,options)

%Fits the normal distribution to the data x based on maximum likelihood.
...
if sum(xlow==xupp)==length(xlow)  %if all data are exactly observed
    [pars(1) pars(2)]=normfit(xlow);
    logL=@(g) -sum(log(normpdf(x,g(1),g(2))));
   % if isempty(lastwarn)==1;exitflag=NaN;end
   gval=-logL(pars);%end
    
elseif  max(xupp)==Inf && min(xlow)>-Inf && isequal(xupp(xupp<Inf),xlow(xupp<Inf))==1 %and if there is ONLY right censoring 
       statusupp=(xupp==Inf);
       [pars(1) pars(2)]=normfit(xupp,0.05,statusupp);
       logL=@(g) -sum((statusupp==0).*log(normpdf(xupp,g(1),g(2)))+...
                (statusupp==1).*log(1-normcdf(xupp,g(1),g(2))));
       ...
       if isempty(init)==1
            ...
            [init(1) init(2)]=normfit(datanew);
            
       end
              %this is the general likeelihood:
              logL=@(g) -  ((sum(-1.*(xlow==xupp)+log(normcdf(xupp,g(1),g(2)) - normcdf(xlow,g(1),g(2))).^(xlow<xupp)))+...
                     sum(-1.*(xlow<xupp)+(log(normpdf(xupp,g(1),g(2)))).^(xlow==xupp)));
