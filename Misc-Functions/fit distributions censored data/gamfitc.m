function [pars covars SE gval exitflag]=gamfitc(x,minimizer,init,options)

%Fits the gamma distribution to the data x based on maximum likelihood.
...
if sum(xlow==xupp)==length(xlow)  %if all data are exactly observed
    pars=gamfit(xlow);
    logL=@(g) -sum(log(gampdf(x,g(1),g(2))));
...
elseif  max(xupp)==Inf && min(xlow)>0 && isequal(xupp(xupp<Inf),xlow(xupp<Inf))==1 %and if there is ONLY right censoring 
       statusupp=(xupp==Inf);
       pars=gamfit(xupp,0.05,statusupp);
       logL=@(g) -sum((statusupp==0).*log(gampdf(xupp,g(1),g(2)))+...
                (statusupp==1).*log(1-gamcdf(xupp,g(1),g(2))));
       %if isempty(lastwarn)==1;exitflag=1;end
       gval=-logL(pars);%end
       ...
       if isempty(init)==1
            ...
            init=gamfit(datanew);
              ...
              logL=@(g) -  ((sum(-1.*(xlow==xupp)+log(gamcdf(xupp,g(1),g(2)) - gamcdf(xlow,g(1),g(2))).^(xlow<xupp)))+...
                     sum(-1.*(xlow<xupp)+(log(gampdf(xupp,g(1),g(2)))).^(xlow==xupp)));
