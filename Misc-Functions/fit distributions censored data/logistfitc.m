function [pars covars SE gval exitflag]=logistfitc(x,minimizer,init,options)

%Fits the logistic distribution to the data x based on maximum likelihood.
%The data can be left and/or right and/or interval censored.
%For the parameterization of this distribution see:
%pdf = (exp((x-theta)/lambda)) / (lambda * (1 + exp((x-theta)/lambda))^2)

%INPUT ARGUMENTS:
%x: a two column matrix of the data. For example

%  -Inf   5.0000  --> left censored observation
...
%-------OUTPUT ARGUMENTS:
%
%pars:     estimated parameters: [theta lambda].
...
%data. 
lcdf=@(x,g) 1./(1+exp(-(x-g(1))./(g(2))));


lastwarn('')
if sum(xlow==xupp)==length(xlow)  %if all data are exactly observed
    pars=mle(xlow,'distribution','logistic');
    logL=@(g) -sum(log(lpdf(x,[g(1) g(2)])));  %-sum(log(lognpdf(x,g(1),g(2))));
    %if isempty(lastwarn)==1;exitflag=1;end
    gval=-logL(pars);%end
    
elseif  max(xupp)==Inf && min(xlow)>-Inf && isequal(xupp(xupp<Inf),xlow(xupp<Inf))==1 %and if there is ONLY right censoring 
       statusupp=(xupp==Inf);
       pars=mle(xupp,'distribution','logistic');
       logL=@(g) -sum((statusupp==0).*log(lpdf(xupp,[g(1) g(2)]))+...
     ...
     gval=-logL(pars);%end
       ...
       if isempty(init)==1
            ...
            datanew=x;
            at= xlow==-Inf | xupp==Inf;
            datanew(at,:)=[];
            datanew=(datanew(:,1)+datanew(:,2))./2;
            init=mle(datanew,'distribution','logistic');
...
logL=@(g) -((sum(((log(lcdf(xupp,[g(1),g(2)]) - lcdf(xlow,[g(1),g(2)]) +(xlow==xupp) ).*(xlow<xupp)))))+...
                     sum((log(lpdf(xupp,[g(1),g(2)]))).^(xlow==xupp)-1.*(xlow<xupp))  );

%                 lcdf(xlow,[2 3])
%                  my1=@(g)  sum(-1.*(xlow<xupp)+(loglpdf(xupp,[g(1),g(2)])).^(xlow==xupp));
%                  my2=@(g) ((sum(-1.*(xlow==xupp)+log(lcdf(xupp,[g(1),g(2)]) - lcdf(xlow,[g(1),g(2)])).^(xlow<xupp))))
...
if minimizer==1
    ...
    if sum(isnan(pars))==2;[pars , gval, exitflag]=fminsearchbnd(logL,init,[-Inf 0],[Inf Inf],options);end
    gval=-gval;
elseif minimizer==2
    if isempty(options)==1;options = optimset('LargeScale','off','Algorithm','sqp','MaxFunEvals',8000,'MaxIter',8000);end
    if sum(isnan(pars))==2;[pars , gval, exitflag , ~ , ~, ~, HESSIANN]=fmincon(logL,init,[],[],[],[],[-Inf 0],[Inf Inf],[],options);end
    gval=-gval;
elseif minimizer==3
...
function out=lpdf(x,g)
at= abs(x)==Inf;
out=(exp((x-g(1))./g(2)))./(g(2).*(1 + exp((x-g(1))./g(2))).^2);
out(at)=0;
end
