function [pars covars SE gval exitflag]=raylfitc(x,minimizer,init,options)

%Fits the rayleigh distribution to the data x based on maximum likelihood.
...
%covars:   variance of the estimated parameter
%SE:       standard error of the estimated parameter
...
pars=NaN;exitflag=NaN;
xlow=x(:,1);xupp=x(:,2);
if min(xlow)<0;error('The data must be non-negative for the Rayleigh distribution');end
if min(xupp)<0;error('The data must be non-negative for the Rayleigh distribution');end
...
elseif  max(xupp)==Inf && min(xlow)>0 && isequal(xupp(xupp<Inf),xlow(xupp<Inf))==1 %and if there is ONLY right censoring 
       statusupp=(xupp==Inf);
       pars=raylfit(xupp,0.05,statusupp);
       logL=@(g) -sum((statusupp==0).*log(raypdf(xupp,g(1)))+...
                (statusupp==1).*log(1-raylcdf(xupp,g(1))));
     %  if isempty(lastwarn)==1;exitflag=1;end
     gval=-logL(pars);%end
       ...
       if isempty(init)==1
            ...
            init=raylfit(datanew);
            
       end
       
              %this is the general likeelihood:
              logL=@(g) -  ((sum(-1.*(xlow==xupp)+log(raylcdf(xupp,g(1)) - raylcdf(xlow,g(1))).^(xlow<xupp)))+...
                     sum(-1.*(xlow<xupp)+(log(raypdf(xupp,g(1)))).^(xlow==xupp)));
...
if minimizer==1
    ...
    if sum(isnan(pars))==1;[pars , gval, exitflag]=fminsearchbnd(logL,init,0,Inf,options);end
    gval=-gval;
elseif minimizer==2
    if isempty(options)==1;options = optimset('LargeScale','off','Algorithm','sqp','MaxFunEvals',8000,'MaxIter',8000);end
    if sum(isnan(pars))==1;[pars , gval, exitflag , ~ , ~, ~, HESSIANN]=fmincon(logL,init,[],[],[],[],0,Inf,[],options);end
    gval=-gval;
elseif minimizer==3
