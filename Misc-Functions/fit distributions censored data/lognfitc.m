function [pars covars SE gval exitflag]=lognfitc(x,minimizer,init,options)

%Fits the lognormal distribution to the data x based on maximum likelihood.
...
xlow=x(:,1);xupp=x(:,2);
if min(xlow)<0;error('The data must be non-negative for the Log-Normal distribution');end
if min(xupp)<0;error('The data must be non-negative for the Log-Normal distribution');end
...
elseif  max(xupp)==Inf && min(xlow)>-Inf && isequal(xupp(xupp<Inf),xlow(xupp<Inf))==1 %and if there is ONLY right censoring 
       statusupp=(xupp==Inf);
       pars=lognfit(xupp,0.05,statusupp);
       logL=@(g) -sum((statusupp==0).*log(lognpdf(xupp,g(1),g(2)))+...
                (statusupp==1).*log(1-logncdf(xupp,g(1),g(2))));
       %  if isempty(lastwarn)==1;exitflag=1;end%
       gval=-logL(pars);%end
       ...
       if isempty(init)==1
            ...
            init=lognfit(datanew);
            
       end
              %this is the general likeelihood:
          logL=@(g)-((sum(-1.*(xlow==xupp)+log(logncdf(xupp,g(1),g(2))...
              -logncdf(xlow,g(1),g(2))).^(xlow<xupp)))+...
              sum(-1.*(xlow<xupp)+(log(lognpdf(xupp,g(1),g(2)))).^(xlow==xupp)));
