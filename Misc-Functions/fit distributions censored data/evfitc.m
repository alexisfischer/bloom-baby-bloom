function [pars covars SE gval exitflag]=evfitc(x,minimizer,init,options)

%Fits the extreme value distribution to the data x based on maximum likelihood.
...
if sum(xlow==xupp)==length(xlow)  %if all data are exactly observed
    pars=evfit(xlow);
    logL=@(g) -sum(log(evpdf(x,g(1),g(2))));
...
elseif  max(xupp)==Inf && min(xlow)>-Inf && isequal(xupp(xupp<Inf),xlow(xupp<Inf))==1 %and if there is ONLY right censoring 
       statusupp=(xupp==Inf);
       pars=evfit(xupp,0.05,statusupp);
       logL=@(g) -sum((statusupp==0).*log(evpdf(xupp,g(1),g(2)))+...
                (statusupp==1).*log(1-evcdf(xupp,g(1),g(2))));
     %  if isempty(lastwarn)==1;exitflag=1;end
     gval=-logL(pars);%end
       ...
       if isempty(init)==1
            ...
            init=evfit(datanew);
              ...
              logL=@(g) -  ((sum(-1.*(xlow==xupp)+log(evcdf(xupp,g(1),g(2)) - evcdf(xlow,g(1),g(2))).^(xlow<xupp)))+...
                     sum(-1.*(xlow<xupp)+(log(evpdf(xupp,g(1),g(2)))).^(xlow==xupp)));
