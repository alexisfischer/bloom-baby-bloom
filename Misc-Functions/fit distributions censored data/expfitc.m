function [pars covars SE gval exitflag]=expfitc(x,minimizer,init,options)

%Fits the exponential distribution to the data x based on maximum likelihood.
...
xlow=x(:,1);xupp=x(:,2);
if min(xlow)<0;error('The data must be non-negative for the Exponential distribution');end
if min(xupp)<0;error('The data must be non-negative for the Exponential distribution');end
...
elseif  max(xupp)==Inf && min(xlow)>0 && isequal(xupp(xupp<Inf),xlow(xupp<Inf))==1 %and if there is ONLY right censoring 
       statusupp=(xupp==Inf);
       pars=expfit(xupp,0.05,statusupp);
       logL=@(g) -sum((statusupp==0).*log(wblpdf(xupp,g(1),1))+...
                (statusupp==1).*log(1-wblcdf(xupp,g(1),1)));
     %  if isempty(lastwarn)==1;exitflag=1;end
     gval=-logL(pars);%end
       ...
       if isempty(init)==1
            ...
            init=expfit(datanew);
              ...
              logL=@(g) -  ((sum(-1.*(xlow==xupp)+log(wblcdf(xupp,g(1),1) - wblcdf(xlow,g(1),1)).^(xlow<xupp)))+...
                     sum(-1.*(xlow<xupp)+(log(wblpdf(xupp,g(1),1))).^(xlow==xupp)));
