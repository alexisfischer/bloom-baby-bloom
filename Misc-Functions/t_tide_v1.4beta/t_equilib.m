...
if nargout==0
     ...
     set(gcf,'defaultaxescolororder',cols(const.doodsonspecies(jk)+3,:),'defaultaxesfontsize',14);
     semilogy(24*[freq,freq]',[repmat(min(amp),length(amp),1) amp]');
     title(sprintf('Tidal Potential at %.1f^\\circ latitude',lat));
     ...
     ylabel('Potential (m)');
     set(gca,'tickdir','out','ylim',[min(amp) max(amp)*2],'box','off');
