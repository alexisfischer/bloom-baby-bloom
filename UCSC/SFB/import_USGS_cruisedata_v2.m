function[S,Si]=import_USGS_cruisedata_v2(filepath,filename,deltaflow,distance,phytoflash,correction,DepthMLD)
%% import USGS cruise data for 2013-present
% Example inputs
%  filepath='/Users/afischer/MATLAB/bloom-baby-bloom/SFB/';
%  filename="/Users/afischer/MATLAB/bloom-baby-bloom/SFB/Data/sfb_raw_1992-present.xlsx";
...
clearvars Ammonium CalculatedChlorophyll CalculatedOxygen CalculatedSPM...
    PhaChl D19 Date Distancefrom36 IFCB Latitude Light Longitude...
...
for i=1:length(S)
    for j=1:length(Mi)        
        if datenum(Mi(j).dn) == datenum(S(i).dn)
            S(i).mld=Mi(j).mld1;
            S(i).salI=Mi(j).salI;
            S(i).chlI=Mi(j).chlI;
        else   
