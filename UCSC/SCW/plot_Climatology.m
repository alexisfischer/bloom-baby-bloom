...
var= SC.DINOrai; varname = 'Dinoflagellate Chl'; [dinoC] = extractClimatology(var,dn,filepath,varname,n,gap);

%%
var = SC.windU; varname = 'Alongshore wind'; [Uwind] = extractClimatology(var,dn,filepath,varname,n,gap);

var = SC.windV; varname = 'Crossshore wind'; [Vwind] = extractClimatology(var,dn,filepath,varname,n,gap);

var = SC.NPGO; varname = 'NPGO'; [NPGO] = extractClimatology(var,dn,filepath,varname,n,gap);

var = SC.MEI; varname = 'MEI'; [MEI] = extractClimatology(var,dn,filepath,varname,n,gap);

var = SC.Tsensor; varname = 'Temperature'; [temp] = extractClimatology(var,dn,filepath,varname,n,gap);

var=log10(SC.sanlorR); var(var<0)=0; varname = 'Discharge'; [sanlorR] = extractClimatology(var,dn,filepath,varname,n,gap);

var = SC.nitrate; varname = 'Nitrate'; [nitrate] = extractClimatology(var,dn,filepath,varname,n,gap);
var = SC.upwell; varname = 'UI'; [UI] = extractClimatology(var,dn,filepath,varname,n,gap);

var = SC.BEUTI; varname = 'BEUTI'; [BEUTI] = extractClimatology(var,dn,filepath,varname,n,gap);
var = SC.CUTI; varname = 'CUTI'; [CUTI] = extractClimatology(var,dn,filepath,varname,n,gap);
...
%% TEST Plot Climatology 2001-2018
...
xax2=datenum('01-Jan-2020');
tick=datenum(2001:1:2020,1,1);
...
set(gca,'xlim',[xax1 xax2],'xgrid','on','fontsize',12,...
    'xtick',tick,'xticklabel',{},'tickdir','out'); box on
ylabel({'Nitrate'},'fontsize',14,'fontweight','bold')
hold on

subplot(all,1,6); anomaly(BEUTI.dn14d,BEUTI.tAnom);
set(gca,'xlim',[xax1 xax2],'xgrid','on','fontsize',12,'xticklabel',{},...
    'xtick',tick,'tickdir','out'); box on
ylabel({'BEUTI'},'fontsize',14,'fontweight','bold')

subplot(all,1,7); anomaly(CUTI.dn14d,CUTI.tAnom);
set(gca,'xlim',[xax1 xax2],'xgrid','on','fontsize',12,'xticklabel',{},...
    'xtick',tick,'tickdir','out'); box on
ylabel({'CUTI'},'fontsize',14,'fontweight','bold')

subplot(all,1,8); anomaly(UI.dn14d,UI.tAnom);
set(gca,'xlim',[xax1 xax2],'xgrid','on','fontsize',12,'xticklabel',{},...
    'xtick',tick,'ylim',[-2 2.5],'tickdir','out'); box on
...
set(gcf,'color','w');
print(gcf,'-dtiff','-r200',[filepath 'Figs\Climatology_SCW_2001-2018.tiff']);
hold off

%% Plot Climatology 2001-2018
figure('Units','inches','Position',[1 1 7.5 9],'PaperPositionMode','auto');
subplot = @(m,n,p) subtightplot (m, n, p, [0.01 0.01], [0.05 0.05], [0.08 0.03]);
%subplot = @(m,n,p) subtightplot(m,n,p,opt{:}); 
%where opt = {gap, width_h, width_w} describes the inner and outer spacings.
...
Uwind.dn14d(1)=[]; Uwind.tAnom(1)=[]; Uwind.dn14d(end)=[]; Uwind.tAnom(end)=[];
subplot(all,1,2); anomaly(Uwind.dn14d,Uwind.tAnom);
set(gca,'xlim',[xax1 xax2],'ylim',[-1.4 3],'ytick',[-1,0,2],'xgrid','on','fontsize',12,'xticklabel',{},...
    'xtick',tick,'tickdir','out'); box on
ylabel({'EW wind'},'fontsize',14,'fontweight','bold')
hold on
...
subplot(all,1,5); anomaly(nitrate.dn14d,nitrate.tAnom);
set(gca,'xlim',[xax1 xax2],'xgrid','on','fontsize',12,...
    'xtick',tick,'xticklabel',{},'ylim',[-2.2 5],'ytick',[-2,0,4],'tickdir','out'); box on
ylabel({'Nitrate'},'fontsize',14,'fontweight','bold')
hold on
...
subplot(all,1,8); anomaly(NPGO.dn14d,NPGO.ti9);
set(gca,'xlim',[xax1 xax2],'xgrid','on','fontsize',12,'ytick',[-2,0,2],...
    'xtick',tick,'xticklabel',{},'tickdir','out'); box on
ylabel({'NPGO'},'fontsize',14,'fontweight','bold')
hold on
...
%C=temp; varname='Temperature'; units='ºC';
...
%% Plot dinoflagellate climatology for NSF proposal

C=dinoC; varname='Dinoflagellate Chl'; units='log Dino Chl (\mug/L)';

%figure('Units','inches','Position',[1 1 8 2],'PaperPositionMode','auto');
figure('Units','inches','Position',[1 1 8 4.5],'PaperPositionMode','auto');
subplot = @(m,n,p) subtightplot (m, n, p, [0.16 0.16], [0.07 0.02], [0.1 0.04]);
%subplot = @(m,n,p) subtightplot(m,n,p,opt{:}); 
%where opt = {gap, width_h, width_w} describes the inner and outer spacings.

xax1=datenum('01-Jan-2001');
xax2=datenum('01-Apr-2020');

subplot(2,1,1);
anomaly(C.dn14d,C.tAnom);
set(gca,'xlim',[xax1 xax2],'ylim',[-2.2 4],'ytick',-2:2:4,'xgrid','on',...
    'fontsize',14)
datetick('x','yyyy','keeplimits');
box on
ylabel({'Standard deviation'; 'from climatology'},'fontsize',14)
hold on

subplot(2,1,2);
h=plot(C.dn14d,C.t14d,':k',C.dn14d,C.ti3,'-k','linewidth',2);
set(h(1),'linewidth',2.5);
set(gca,'xlim',[datenum('01-Jan-2017') xax2],...
    'ylim',[0 1.7],'ytick',0:.5:1.5,'yticklabel',{'0','.5','1','1.5'},...
    'xgrid','on','fontsize',14)
datetick('x','yyyy','keeplimits')
ylabel({['' num2str(units) '']},'fontsize',14)
legend('climatology','raw data','Location','N')
legend boxoff
hold on

% set figure parameters
set(gcf,'color','w');
print(gcf,'-dtiff','-r100',[filepath 'Figs\AnomalyClimatology_SCW_Dino.png']);
hold off
