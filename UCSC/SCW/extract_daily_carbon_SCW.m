function [phytoTotal,dino,diat,nano,zoop,carbonml,classbiovol,class2useTB,mdate] = extract_daily_carbon_SCW(biovol_path)
...
carbonml=carbonml*1000; %need to multiply by 1000 bc must have integers for randi
 
if yr == 2018
  %  itime(1).dn=find(mdate>=datenum('27-Feb-2018') & mdate<=datenum('09-Mar-2018')); 
    itime(1).dn=find(mdate>=datenum('05-Apr-2018') & mdate<=datenum('14-Apr-2018')); 
    itime(2).dn=find(mdate>=datenum('10-Apr-2018') & mdate<=datenum('21-Apr-2018')); 
    itime(3).dn=find(mdate>=datenum('24-Aug-2018') & mdate<=datenum('27-Aug-2018')); 
    itime(4).dn=find(mdate>=datenum('16-Sep-2018') & mdate<=datenum('30-Sep-2018')); 
...
carbonml=carbonml./1000; %divide by 1000 to remove it
...
clearvars time idx i j max3 mdateTB classbiovolTB classcountTB cellC...
