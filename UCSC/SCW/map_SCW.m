...
%% plot Monterey bay for NSF proposal schematic
...
m_proj('albers equal-area','lat',[36.57 37.14],'long',[-122.35 -121.77]);

m_gshhs_f('patch',[.7 .7 .7],'edgecolor','none');  
m_grid('linestyle','none','linewidth',1,'tickdir','out',...
     'xaxisloc','top','yaxisloc','left','fontsize',13,'backcolor',C(1,:));  

%m_line(-122.33,37.112,'marker','.','markersize',16,'color','k'); %Ano Nuevo
%m_line(-122.017,36.96,'marker','.','markersize',16,'color','k'); %SCMW
%m_line(-122.029,36.751,'marker','.','markersize',16,'color','k'); %M1

% set figure parameters
set(gcf,'color','w');
print(gcf,'-dtiff','-r600',[filepath 'SCW/Figs/MB_map.tif']);
hold off 

%% plot Monterey bay for Fischer et al. 2020
