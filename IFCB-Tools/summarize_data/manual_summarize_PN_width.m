...
[~,~,it]=intersect(find(classlist(:,2)==find(strcmp('Pseudo-nitzschia_large_3cell',class2use_manual))),targets.roi_number); Lcell3=targets.MinorAxisLength(it)';
[~,~,it]=intersect(find(classlist(:,2)==find(strcmp('Pseudo-nitzschia_large_4cell',class2use_manual))),targets.roi_number); Lcell4=targets.MinorAxisLength(it)';
large_PN=size(Lcell1,2)+2*size(Lcell2,2)+3*size(Lcell3,2)+4*size(Lcell4,2);
...
[~,~,it]=intersect(find(classlist(:,2)==find(strcmp('Pseudo-nitzschia_small_3cell',class2use_manual))),targets.roi_number); Scell3=targets.MinorAxisLength(it)';
[~,~,it]=intersect(find(classlist(:,2)==find(strcmp('Pseudo-nitzschia_small_4cell',class2use_manual))),targets.roi_number); Scell4=targets.MinorAxisLength(it)';
small_PN=size(Scell1,2)+2*size(Scell2,2)+3*size(Scell3,2)+4*size(Scell4,2);
