function [class2do_string,mdate,y_mat,iraicar,iraifx,fish,mcrpy,RAI] = matchClassifierwMicroscopy(filepath,class2do_string,chain,error,yr)
...
[cellsmL,~,mdate] = extract_daily_cellsml([filepath 'Data/IFCB_summary/class/summary_allTB_' yr]);
[phytoTotal,~,~,~,~,carbonml,~,class2useTB,~] = extract_daily_carbon([filepath 'Data/IFCB_summary/class/summary_biovol_allTB' yr]);
...
load([filepath 'Data/RAI'],'DN','class','RAI','FX');
...
clearvars id idx iD iPn iAlex iDphy n errR dnF errF F SC chain error yr class2useTB error filepath
