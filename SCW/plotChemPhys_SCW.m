...
subplot(3,1,1); %46042
    [U,~]=plfilt(W.b46042(7).U,W.b46042(7).DN);
    [V,DN]=plfilt(W.b46042(7).V,W.b46042(7).DN);
    [~,u,~] = ts_aggregation(DN,U,1,'6hour',@mean);
    [time,v,~] = ts_aggregation(DN,V,1,'6hour',@mean);
...
subplot(3,1,2); %M1
    [U,~]=plfilt(W.M1(3).U,W.M1(3).DN);
    [V,DN]=plfilt(W.M1(3).V,W.M1(3).DN);
    [~,u,~] = ts_aggregation(DN,U,1,'6hour',@mean);
    [time,v,~] = ts_aggregation(DN,V,1,'6hour',@mean);
    xax1=datenum('2018-01-01'); xax2=datenum('2018-07-01');
    yax1=-10; yax2=10;
...
for i=1:length(W.b46042)
    subplot(length(W.b46042),1,i)
    
    [U,~]=plfilt(W.b46042(i).U,W.b46042(i).DN);
    [V,DN]=plfilt(W.b46042(i).V,W.b46042(i).DN);
    [~,u,~] = ts_aggregation(DN,U,1,'4hour',@mean);
    [time,v,~] = ts_aggregation(DN,V,1,'4hour',@mean);
        
    xax1=datenum(['' num2str(W.b46042(i).yr) '-01-01']);
    xax2=datenum(['' num2str(W.b46042(i).yr) '-12-31']);
    yax1=-10; yax2=10;
    stick(time,u,v,xax1,xax2,yax1,yax2,'');
    legend(['' num2str(W.b46042(i).yr) ''],'Location','NW'); legend boxoff
    hold on
end
...
for i=1:length(W.SC)
    subplot(length(W.SC),1,i)
    
    [U,~]=plfilt(W.SC(i).U,W.SC(i).DN);
    [V,DN]=plfilt(W.SC(i).V,W.SC(i).DN);
    [~,u,~] = ts_aggregation(DN,U,1,'8hour',@mean);
    [time,v,~] = ts_aggregation(DN,V,1,'8hour',@mean);
        
    xax1=datenum(['' num2str(W.SC(i).yr) '-01-01']);
    xax2=datenum(['' num2str(W.SC(i).yr) '-12-31']);
    yax1=-4; yax2=4;
    stick(time,u,v,xax1,xax2,yax1,yax2,'');
    legend(['' num2str(W.SC(i).yr) ''],'Location','NW'); legend boxoff
    hold on  
end
...
[U,~]=plfilt(W.M1(1).U,W.M1(1).DN);
[V,DN]=plfilt(W.M1(1).V,W.M1(1).DN);
...
plot(W.M1(1).dn,smooth(W.M1(1).T),'-','color',[0,0.4470,0.7410]);
datetick('x','m');
...
[U,~]=plfilt(W.M1(2).U,W.M1(2).DN);
[V,DN]=plfilt(W.M1(2).V,W.M1(2).DN);
[~,u,~] = ts_aggregation(DN,U,1,'day',@mean);
[time,v,~] = ts_aggregation(DN,V,1,'day',@mean);
...
plot(W.M1(2).dn,smooth(W.M1(2).T),'-','color',[0,0.4470,0.7410]);
datetick('x','m');
axis([xax1 xax2 10 16]);
set(gca,'xgrid','on','xticklabel',{},'ytick',10:3:16,'tickdir','out');
ylabel('SST (^oC)','fontsize',12, 'fontname', 'Arial');    
...
plot(W.M1(3).dn,smooth(W.M1(3).T),'-','color',[0,0.4470,0.7410]);
datetick('x','m');
axis([xax1 xax2 10 16]);
