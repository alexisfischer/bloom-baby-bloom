...
load([filepath 'SCW/Data/Hfr_daily_SCW_March2018'],'dn','u','v','lat','lon','iilat','iilon')
load([filepath 'SCW/Figs/coast_montereybay'],'ncst');
    
%% Test figure of timepoint: quivermc
itime=find(dn==datenum('18-Mar-2018'));
...
itime=find(dn==datenum('01-Mar-2018')); C=brewermap(4,'RdBu');
...
itime=find(dn==datenum('05-Mar-2018')); C=brewermap(4,'RdBu');
U=squeeze(u(:,:,itime))'; V=squeeze(v(:,:,itime))'; LON=lon'; LAT=lat';
...
itime=find(dn==datenum('20-Mar-2018'));
U=squeeze(u(:,:,itime))'; V=squeeze(v(:,:,itime))'; LON=lon'; LAT=lat';
...
hold on
[sx,sy]=meshgrid(iilon(1):.04:iilon(2),36.75);
