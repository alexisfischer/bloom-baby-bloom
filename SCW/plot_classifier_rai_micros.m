...
% class2do_string = 'Prorocentrum';chain=1; %not great
...
yyaxis left
    ...
    h3=errorbar(DNm,M,ERR,'s','Color',col(3,:),'Linestyle','none',...
        'MarkerFaceColor',col(3,:),'Linewidth',1,'Markersize',5); %microscopy
    ...
    if idx == 19 %FISH
        h4 = errorbar(dnF,Pnml,err_Pn,'d','Linestyle','none','Linewidth',1,...
            'Color',col(2,:),'MarkerFaceColor',col(2,:),'Markersize',5); hold on  
...
yyaxis right
    h5=errorbar(dnr,rai,neg,pos,'^','Color',col(4,:),'Linewidth',1,...
        'Linestyle','none','Markersize',4); hold on
    set(gca,'ylim',[0 100],'ytick',0:25:100,...
        'xlim',[xax1 xax2],'fontsize',12,'ycolor',col(4,:),'tickdir','out');  
    datetick('x','m','keeplimits');
    ylabel('Relative percent of cells','fontsize',13,'fontweight','bold');  
    hold on
    
    if idx == 19
        lh = legend([h1 h2 h3 h5 h4],'Classifier','Manual','Microscopy','RAI','FISH','location','NE');
    elseif idx == 37
        lh = legend([h1 h2 h3 h5],'Classifier','Manual','Microscopy','RAI','location','NW');
    else
        lh = legend([h1 h2 h3 h5],'Classifier','Manual','Microscopy','RAI','location','NE');    
    end    
